...
namespace objl
{
	...
	public:
		...
		bool LoadFile(std::string Path)
		{
			...
			while (std::getline(file, curline))
			{
				...
				if (algorithm::firstToken(curline) == "o" || algorithm::firstToken(curline) == "g" || curline[0] == 'g')
				{
					...
					else
					{
						...
						if (!Indices.empty() && !Vertices.empty())
						{
							...
							// Insert Mesh
							loaded_meshes_.push_back(tempMesh);
				...
				if (algorithm::firstToken(curline) == "f")
				{
					...
					for (int i = 0; i < int(vVerts.size()); i++)
					{
						...
						loaded_vertices_.push_back(vVerts[i]);
					}
					...
					for (int i = 0; i < int(iIndices.size()); i++)
					{
						...
						indnum = (unsigned int)((loaded_vertices_.size()) - vVerts.size()) + iIndices[i];
						loaded_indices_.push_back(indnum);
			...
			for (int i = 0; i < MeshMatNames.size(); i++)
			{
				...
				for (int j = 0; j < LoadedMaterials.size(); j++)
				{
					...
					{
						loaded_meshes_[i].MeshMaterial = LoadedMaterials[j];
						break;
					}
				}
			}

			if (loaded_meshes_.empty() && loaded_vertices_.empty() && loaded_indices_.empty())
			{
				return false;
			}
			else
		...
		std::vector<unsigned int> loaded_indices_;
